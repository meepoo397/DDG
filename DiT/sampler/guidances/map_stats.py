...
class MapStatsSampler_x0pred(SamplerWithClassifier):
    def __init__(
            self,
            net,
            gnet,
    ...
    def denoise_with_candidate(self, x: torch.Tensor, sigma: torch.Tensor, labels: torch.Tensor, i: int,seed: torch.Tensor,candidates:list,activate_map):
        ...
        ref_Dx_map = torch.stack(ref_Dx_list, dim=0).mean(dim=0)
        ref_Dx_cfg = self.gnet(x, sigma, labels).to(self.dtype)
        # print(f"ref_Dx_map.shape:{ref_Dx_map.shape}")
        # print(f"ref_Dx_cfg.shape:{ref_Dx_cfg.shape}")
        # print(f'activate_map.shape:{activate_map.shape}')
        ref_Dx = torch.where(activate_map.view(-1, 1, 1, 1), ref_Dx_map, ref_Dx_cfg)
