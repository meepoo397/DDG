...
def open_image_folder(source_dir, *, max_images: Optional[int], labels_csv: Optional[str]) -> tuple[int, Iterator[ImageEntry]]:
    input_images = []
    ...
    if os.path.isfile(meta_fname):
        ...
        print("Didn't found dataset.json but try to build labels by csv.")
        # print('arch_fnames:',arch_fnames)
        image_fnames_noext = {fname:fname.split('.')[0] for fname in arch_fnames.values()}
        
        try:
            val_labels = pd.read_csv(labels_csv)
        except:
            raise Exception(f"Unable to open labels_csv:{labels_csv}")
        val_map = dict()
        csv_map = {row['ImageId']:row['PredictionString'].split()[0] for i,row in val_labels.iterrows()}
        # print('csv_map:',csv_map)
        
        toplevel_names = {fname_noext: csv_map[fname_noext] for fname_noext in image_fnames_noext.values()}
        # print('toplevel_names:',toplevel_names)
        # print(len(set(toplevel_names.values())))

        toplevel_indices = {toplevel_name: idx for idx, toplevel_name in enumerate(sorted(set(toplevel_names.values())))}
        # print('toplevel_indices:',toplevel_indices)
        labels = {fname: toplevel_indices[toplevel_names[fname_noext]] for fname, fname_noext in image_fnames_noext.items()}
        # print('labels:',labels)
    else:
        print("Didn't found dataset.json and didn't pass csv path.")
    ...
    assert len(toplevel_indices)==1000
    def iterate_images():
...
def make_transform(
    ...
    def DiT_crop(image_size:int,arr:np.ndarray):
        pil_image = PIL.Image.fromarray(arr)
    ...
    if transform == 'DiT_crop':
        return functools.partial(DiT_crop, output_width)
        
    assert False, 'unknown transform'

#----------------------------------------------------------------------------

def open_dataset(source, *, max_images: Optional[int], labels_csv=Optional[str]):
    if os.path.isdir(source):
        return open_image_folder(source, max_images=max_images, labels_csv=labels_csv)
    elif os.path.isfile(source):
        if file_ext(source) == 'zip':
            return open_image_zip(source, max_images=max_images)#暫不支援zip使用labels csv
        else:
...
@click.option('--transform',  help='Input crop/resize mode', metavar='MODE',            type=click.Choice(['center-crop', 'center-crop-wide', 'center-crop-dhariwal','DiT_crop']))
@click.option('--resolution', help='Output resolution (e.g., 512x512)', metavar='WxH',  type=parse_tuple)
@click.option('--labels-csv',  help='Path of the csv of information in labels', metavar='PATH',  type=str)
...
def convert(
    ...
    num_files, input_iter = open_dataset(source, max_images=max_images, labels_csv=labels_csv)
...
def convert_encode(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        img_tensor = torch.tensor(img).to('cuda').permute(2, 0, 1).unsqueeze(0)
        mean_std = vae.encode_pixels(img_tensor)[0].cpu()
        idx_str = f'{idx:08d}'
        archive_fname = f'{idx_str[:5]}/img-mean-std-{idx_str}.npy'
