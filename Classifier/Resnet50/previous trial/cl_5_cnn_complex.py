...
def create_resnet50_classifier(num_classes, in_channels, pretrained,**other_kwargs):
    ...
    class VAEClassifierResNet(nn.Module):
        def __init__(self, num_classes=1000,in_channels=in_channels):
            super().__init__()
            ...
            self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2) # 32x32
    
            # 殘差塊層
            self.layer1 = self._make_layer(32, 64, num_blocks=2, stride=2)  # 16x16
            self.layer2 = self._make_layer(64, 128, num_blocks=2, stride=2) # 8x8
            self.layer3 = self._make_layer(128, 256, num_blocks=2, stride=2)# 4x4
            self.layer4 = self._make_layer(256, 512, num_blocks=2, stride=2)# 2x2
        ...
        def forward(self, x):
            ...
            x = self.fc(x)
            return x
    return VAEClassifierResNet(num_classes=num_classes,in_channels=in_channels)
