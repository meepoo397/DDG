...
def create_efficientnet_classifier(num_classes, in_channels, pretrained, **other_kwargs):
    # """
    # 根據參數創建和修改 EfficientNetV2-L 模型，以適應不同輸入通道數。

    # Args:
    #     num_classes (int): 分類的總類別數。
    #     in_channels (int): 輸入圖片的通道數，例如 3 (RGB) 或 4 (潛在空間)。
    #     pretrained (bool): 是否使用 ImageNet 預訓練權重。
    
    # Returns:
    #     torch.nn.Module: 修改後的 EfficientNetV2-L 模型。
    # """
    # # 載入預訓練的 EfficientNetV2-L 模型
    # if pretrained:
    ...
    # # 修改第一個卷積層的輸入通道數
    # if in_channels != 3:
    #     # 取得 EfficientNet 的第一個卷積層
    #     original_conv = model.features[0][0]
        
    #     # 創建一個新的卷積層，輸入通道數為 in_channels
    #     new_conv = nn.Conv2d(
    #         in_channels,
    ...
    #         # 將原始 3 通道的權重複製到新卷積層的前 3 個通道
    #         new_conv.weight.data[:, :3, :, :] = original_weights
            
    #         # 如果新通道數大於 3，將新通道的權重初始化為原始權重的平均值
    #         if in_channels > 3:
    #             avg_weight = torch.mean(original_weights, dim=1, keepdim=True)
    #             new_conv.weight.data[:, 3:in_channels, :, :] = avg_weight.expand(-1, in_channels - 3, -1, -1)
        
    #     # 將新卷積層替換掉模型的第一個卷積層
    #     model.features[0][0] = new_conv

    # # 修改最後一層全連接層（分類器）
    # # EfficientNetV2 的分類器是 Sequential 模組，包含兩個層
    # in_features = model.classifier[1].in_features
    # model.classifier = nn.Sequential(
    #     nn.Dropout(p=0.2),  # EfficientNetV2-L 預設的 Dropout
    #     nn.Linear(in_features, num_classes) # 直接修改為 num_classes
    # )
...
def main():
    ...
    # 建立模型
    model = create_efficientnet_classifier(args.num_classes, args.in_channels, args.pretrained)
