...
def validate(model, encoder, device,batch_size, valid_loader, criterion,batch_num_valid):
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(batch_num_valid//batch_size),total =batch_num_valid//batch_size):
            ...
            mean, std = images_meanstd.to(torch.float32).chunk(2, dim=1)
            images_latent = mean + torch.randn_like(mean) * std
            images_latent = images_latent.mul_(0.18215)
            ...
            outputs = model(images_latent,dummy_t)
...
def main():
    ...
    ref_mean, ref_std = torch.as_tensor(ref_image).to(device).unsqueeze(0).to(torch.float32).chunk(2, dim=1)
    ref_image = ref_mean + torch.randn_like(ref_mean) * ref_std
    ref_image = ref_image.mul_(0.18215)
