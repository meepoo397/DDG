...
class CFGAdaptiveSamplerV11(SamplerWithClassifier):
    def __init__(
            self,
            net,
            gnet,
    ...
    def sample(self, noise: torch.Tensor, labels: torch.Tensor, seeds: list):
        ...
        for i, (t_cur, t_next,seed) in enumerate(zip(t_steps[:-1], t_steps[1:],seeds_array)): # 0, ..., N-1
            ...
            candidate,clasifier_stats,w_t,average_true_prob = self.calculate_candidate(x_cur,t_cur,labels,i,adaptive_scheduler_state,average_true_prob)
