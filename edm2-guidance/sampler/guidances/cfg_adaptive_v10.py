...
class CFGAdaptiveSamplerV10(SamplerWithClassifier):
    def __init__(
            self,
            net,
            gnet,
    ...
    def denoise_with_candidate(self, x: torch.Tensor, sigma: torch.Tensor, labels: torch.Tensor, i: int,seed: torch.Tensor,candidates:list,activate_cfg,adaptive_scheduler_state):
        ...
        steps_diff = (self.hold_steps - accumulate_hold_steps)/self.hold_steps
        linear_interpolate_guidance = (scheduled_guidance-1)*steps_diff + 1
        # print(f'self.hold_steps:{self.hold_steps}')
        # print(accumulate_hold_steps[:5])
        # print(state_2_mask[:5])
        # print(steps_diff[:5])
        # print(linear_interpolate_guidance[:5])
        scheduled_guidance_each_samples = torch.where(state_2_mask,linear_interpolate_guidance,scheduled_guidance_each_samples)
