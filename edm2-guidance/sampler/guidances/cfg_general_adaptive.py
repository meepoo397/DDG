...
class CFGAdaptiveSamplerGeneral(SamplerWithClassifier):
    def __init__(
            self,
            net,
            gnet,
    ...
    def calculate_candidate(self,x:torch.Tensor,sigma:torch.Tensor,true_labels:torch.Tensor,i,prev_average_true_prob):
        ...
        device = x.device
        state = torch.full((x.shape[0],), 0, dtype=torch.int32,device=device, requires_grad=False).to(device)
        accumulate_hold_steps = torch.full(state.shape, 0, dtype=torch.int32,device=device, requires_grad=False).to(device)
        ...
        open_prob = self.open_prob - self.balance_coeff*diff_average_true_prob
        close_prob = self.close_prob - self.balance_coeff*diff_average_true_prob
        ...
        state = torch.where(transition_0_to_1_mask,torch.full(state.shape, 1, dtype=torch.int32,device=x.device, requires_grad=False),state)
        state = torch.where(transition_1_to_2_mask,torch.full(state.shape, 2, dtype=torch.int32,device=x.device, requires_grad=False),state)
        state = torch.where(transition_2_to_1_mask,torch.full(state.shape, 1, dtype=torch.int32,device=x.device, requires_grad=False),state)
        state = torch.where(transition_2_to_0_mask,torch.full(state.shape, 0, dtype=torch.int32,device=x.device, requires_grad=False),state)

        adaptive_scheduler_state = torch.full((state.shape[0],2), 0, dtype=torch.int32,device=device, requires_grad=False).to(device)
        
        adaptive_scheduler_state[:,0] = state
        adaptive_scheduler_state[:,1] = accumulate_hold_steps
        
        guidance_coeff = self.balance_coeff*diff_average_true_prob
        ...
        return top_ks.tolist(),result_tensor,guidance_coeff,average_true_prob
    ...
    def sample(self, noise: torch.Tensor, labels: torch.Tensor, seeds: list):
        ...
        for i, (t_cur, t_next,seed) in enumerate(zip(t_steps[:-1], t_steps[1:],seeds_array)): # 0, ..., N-1
            ...
            candidate,clasifier_stats,guidance_coeff,average_true_prob = self.calculate_candidate(x_cur,t_cur,labels,i,average_true_prob)
            ...
            # Euler step.
            d_cur = (x_hat - self.denoise_with_candidate(x_hat, t_hat, labels, i,seed,guidance_coeff)) / t_hat
            ...
            if i < self.num_steps - 1:
                if self.heun_guid:
                    d_prime = (x_next - self.denoise_with_candidate(x_next, t_next, labels, i+1,seed,guidance_coeff)) / t_next
                else:
    ...
    def denoise_with_candidate(self, x: torch.Tensor, sigma: torch.Tensor, labels: torch.Tensor, i: int,seed: torch.Tensor,guidance_coeff):
        ...
        debug_print(self.debug,f"scheduled_guidance: {scheduled_guidance},tuned_guidance:{(scheduled_guidance-1)*guidance_coeff+1},guidance_coeff:{guidance_coeff}")
        # 新版切換機制
        ...
        return ref_Dx.lerp(Dx, (scheduled_guidance-1)*guidance_coeff+1)