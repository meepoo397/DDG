...
try:
    symbolic_assert = torch._assert  # PyTorch 1.8.0a0
except AttributeError:
    try:
        symbolic_assert = torch.Assert  # PyTorch 1.7.0（實際上不存在，但保留原邏輯）
    except AttributeError:
        try:
            import torch.jit
            symbolic_assert = torch.jit._assert  # PyTorch 1.10+（TorchScript tracing 時使用）
        except (ImportError, AttributeError):
            # 最後的 fallback，使用普通 assert
            def symbolic_assert(cond, msg=''):
                assert cond, msg
