    ...
    **sampler_kwargs,                           # Additional arguments for the sampler function.
    ...
    if classifier_path is not None:
        ...
        # cl_9
            ...
            classifier.load_state_dict(checkpoint['model_state_dict'])
            classifier.eval()
        elif classifier_path=='/data/guidance-team-new/classifier_log_trained_by_train_pure_first/model085400.pt':
            print(f'Successfully match classifier path and structure:cl_9,{classifier_path}')
            classifier = EncoderUNetModel(
                image_size=64,
                in_channels=4,
                    ...
                    if outdir is not None:
                        for seed, image in zip(batch.seeds, batch.images.permute(0, 2, 3, 1).cpu().numpy()):
                            ...
                            PIL.Image.fromarray(image, 'RGB').save(
                                os.path.join(image_dir, f'{seed:06d}.jpg'),
                                format='JPEG',
                                quality=95
                            )
...
@click.option('--min_guidance',       help='ex:True', metavar='FLOAT',                         type=float, default=1.75, show_default=True)
@click.option('--max_guidance',       help='ex:True', metavar='FLOAT',                         type=float, default=2.8, show_default=True)
@click.option('--balance_coeff',       help='ex:True', metavar='FLOAT',                         type=float, default=11.0, show_default=True)
@click.option('--curve_data',       help='ex:True', metavar='STR',                         type=str, default='edm2_s_cfg_interval_10000', show_default=True)
